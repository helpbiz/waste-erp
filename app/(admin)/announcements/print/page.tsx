...
import AnnouncementsPrintClient from './_print-client';

export const dynamic = 'force-dynamic';

const ADMIN_ROLES = new Set(['SUPER_ADMIN', 'MUNI_ADMIN', 'CONTRACTOR_ADMIN', 'INTERNAL_ADMIN']);

export default async function AnnouncementsPrintPage({
  searchParams,
}: {
  searchParams?: { from?: string; to?: string; autoprint?: string; id?: string };
}) {
  ...
  let companyName = 'WCI 클린 ERP';
  if (session.contractorId) {
    const contractor = await prisma.contractor.findUnique({
      where: { id: BigInt(session.contractorId) },
      select: { companyName: true },
    });
    if (contractor?.companyName) companyName = contractor.companyName;
  }

  return <AnnouncementsPrintClient from={from} to={to} autoprint={autoprint} singleId={singleId} companyName={companyName} />;
}
