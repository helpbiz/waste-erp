  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-mono font-bold text-ink-faint">공유 App(bbegi.com) 앱 연동</span>
      ...
      <div className="bg-surface border border-line rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 flex items-center gap-2">
            ...
            {testResult && (
              <span className={`text-sm font-extrabold px-2 py-1 rounded border-2 ${
        ...
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody className="divide-y divide-line">
            {recentImports.length === 0 && (
              <tr><td colSpan={7} className="px-3 py-8 text-center text-ink-faint">실행 이력이 없습니다.</td></tr>
            )}
            {recentImports.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50">
                <td className="px-3 py-1.5 font-mono text-[0.6875rem]">{new Date(r.createdAt).toLocaleString('ko-KR')}</td>
                <td className="px-3 py-1.5 text-sm font-bold">{r.triggerType === 'cron' ? 'CRON' : '수동'}</td>
        ...
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody className="divide-y divide-line">
            {recentComplaints.length === 0 && (
              <tr><td colSpan={5} className="px-3 py-8 text-center text-ink-faint">등록된 대형폐기물 민원이 없습니다.</td></tr>
            )}
            {recentComplaints.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50">
                <td className="px-3 py-1.5 font-mono text-[0.6875rem]">{new Date(c.reportedAt).toLocaleString('ko-KR')}</td>
                <td className="px-3 py-1.5 text-sm">{c.citizenName ?? '—'}</td>
                <td className="px-3 py-1.5 text-sm max-w-[180px] truncate" title={c.locationAddress ?? ''}>{c.locationAddress ?? '—'}</td>
                <td className="px-3 py-1.5 text-sm max-w-[300px] truncate" title={c.description ?? ''}>{c.description ?? '—'}</td>
                <td className="px-3 py-1.5">
                  <span className={`text-[0.625rem] font-extrabold px-1.5 py-0.5 rounded border ${
      ...
      <div className="text-[0.6875rem] font-mono text-ink-faint px-4 py-2 bg-slate-50 rounded">
