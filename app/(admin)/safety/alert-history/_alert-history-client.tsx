    ...
    <div className="space-y-4">
      ...
      {!loading && (
        <div className="bg-surface border border-line rounded-xl overflow-hidden">
          {items.length === 0 ? (
            <div className="py-12 text-center text-slate-500 text-sm">발송이력이 없습니다.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-line">
                <tr>
                  <th className="px-4 py-2 text-left font-extrabold text-xs">발송일시</th>
                  <th className="px-4 py-2 text-left font-extrabold text-xs">유형</th>
                  <th className="px-4 py-2 text-center font-extrabold text-xs">수신인원</th>
                  <th className="px-4 py-2 text-center font-extrabold text-xs">발송</th>
                  <th className="px-4 py-2 text-center font-extrabold text-xs">실패</th>
                  <th className="px-4 py-2 text-left font-extrabold text-xs hidden md:table-cell">발송자</th>
                  <th className="px-4 py-2 text-left font-extrabold text-xs hidden lg:table-cell">수단</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-surface-soft transition">
                      ...
                      <td className="px-4 py-2">
                        ...
                        <span className="ml-1 text-xs font-bold">{m?.typeLabel ?? '기타'}</span>
                      </td>
                      ...
                      <td className="px-4 py-2 text-center">
                        <span className={`font-bold text-sm ${(m?.failed ?? 0) > 0 ? 'text-red-600' : 'text-slate-400'}`}>
                          {m?.failed ?? '—'}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-xs text-ink-muted hidden md:table-cell">{item.actorName}</td>
                      <td className="px-4 py-2 text-xs text-ink-muted hidden lg:table-cell">{m?.provider ?? '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
