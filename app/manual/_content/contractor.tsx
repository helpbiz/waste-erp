...
export const CONTRACTOR_CHAPTERS: ChapterData[] = [
  ...
  /* ─── 02 처음 로그인 + 콘솔 둘러보기 ─── */
  {
    ...
    steps: [
      ...
      { title: '아이디·임시 비밀번호로 로그인 후 새 비밀번호로 변경', body: '운영팀이 알려드린 아이디(예: HQ001)와 임시 비밀번호로 처음 로그인하시고, 본인만 아는 새 비밀번호로 변경합니다. 이후 모든 로그인은 새 비밀번호로 하시면 됩니다.' },
      {
        ...
        body: '메뉴는 4그룹으로 정리되어 있습니다 — OVERVIEW(대시보드) · CORE MODULES(민원·안전·건강) · OPERATIONS(근태·차량·실적·보고서) · SETTINGS(사용자·대형폐기물·공지).',
        screenshot: (
  ...
  /* ─── 12 추가 기능: 보건기록·원가급여·대형폐기물 ─── */
  {
    ...
    steps: [
      ...
      { title: '원가급여 — 월별 인건비 자동 정산', body: '근태 기록을 기반으로 월별 근무일수·초과근무·야간근무 시간이 자동 집계됩니다. 월말 "최종화" 버튼으로 잠금 후 외부 급여 시스템으로 export 가능합니다.' },
      {
