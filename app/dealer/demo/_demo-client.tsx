...
export default function DemoClient() {
  ...
  const quotaReached = items.filter((i) => (daysLeft(i.expiresAt) ?? 0) > 0).length >= 3;
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      ...
      <button
        onClick={onProvision}
        disabled={provisioning || quotaReached}
        className="mb-2 rounded bg-blue-600 px-4 py-2 text-white disabled:opacity-50"
      >
        {provisioning ? '발급 중…' : '데모 즉시 발급'}
      </button>
