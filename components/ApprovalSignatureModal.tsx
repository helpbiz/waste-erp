    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl w-[520px] max-w-[95vw] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-3 border-b border-line flex items-center">
          ...
          {subtitle && <span className="ml-2 text-[0.6875rem] font-mono text-ink-faint">{subtitle}</span>}
          <button onClick={onClose} className="ml-auto text-ink-faint hover:text-ink text-xl">×</button>
        </div>
        <div className="p-5 space-y-4">
          {preview && <div className="bg-slate-50 border border-line rounded-md px-3 py-2 text-sm">{preview}</div>}

          {storedSignatureUrl && (
            <div>
              <label className="flex items-center gap-2 mb-2">
                <input type="radio" checked={mode === 'stored'} onChange={() => setMode('stored')} />
                <span className="text-sm font-extrabold text-ink">저장된 서명 사용</span>
                {storedSignatureRef && <span className="text-[0.625rem] font-mono text-ink-faint">ref: {storedSignatureRef}</span>}
              </label>
              {mode === 'stored' && (
          ...
          <div>
            <label className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-extrabold text-ink">{storedSignatureUrl ? '재서명' : '서명 작성'}</span>
            </label>
            {mode === 'redraw' && <SignaturePad onChange={setDrawn} height={140} />}
          </div>

          <div>
            <div className="text-[0.625rem] font-mono font-extrabold text-ink-faint mb-1">의견 (선택)</div>
            <input
              value={comment}
