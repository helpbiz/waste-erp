    ...
    <div className="space-y-2">
        ...
        {photos.map((src, i) => (
          <div key={i} className="relative group aspect-square rounded-lg overflow-hidden border-2 border-line bg-surface-soft">
            ...
            <button
              type="button"
              onClick={() => removeAt(i)}
              className="absolute top-1 right-1 w-6 h-6 rounded-full bg-red-600/90 text-white text-sm font-extrabold leading-none hover:bg-red-700 shadow"
              aria-label="사진 제거"
            >
              ×
            </button>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex-1 px-3 py-2 rounded-md bg-accent text-white text-sm font-extrabold hover:bg-accent-strong active:bg-cyan-800 disabled:opacity-50 flex items-center justify-center gap-1.5"
        >
        ...
        <button
          ...
          disabled={loading || photos.length >= max}
          onClick={() => fileRef.current?.click()}
          className="flex-1 px-3 py-2 rounded-md border-2 border-line text-sm font-extrabold hover:bg-surface-soft text-ink disabled:opacity-50 flex items-center justify-center gap-1.5"
        >
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
